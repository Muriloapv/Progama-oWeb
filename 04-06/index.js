...
function addTask(event) {
  ...
  li.innerHTML = `
    ...
    <button  title="Excluir tarefa" onClick="removeElement(${taskId})">❌</button>
  `
...
function removeElement(taskId){
  ...
  var elementoPai = document.getElementById('taskList')
  var elementoRemovido = document.getElementById(`id-${taskId}`)

  elementoPai.removeChild(elementoRemovido);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  taskList.innerHTML = tasks
    ...
    .map(
      (task) => `
      <li id='id-${task.id}'>
        <button title="Excluir tarefa" onClick="removeElement(${task.id})">❌</button>
      </li>
    `
    )
    .join('')   
